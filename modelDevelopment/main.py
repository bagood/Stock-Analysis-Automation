...
from modelDevelopment.helper import _calculate_classification_metrics, _calculate_gini
...
def measure_model_performance(model, feature: np.array, target: np.array, dataset_name: str, positive_label: str, negative_label: str) -> dict:
    """
    ...
    logging.info(f"{dataset_name} Set Performance Summary - Gini: {all_metrics['Gini'][0]:.4f}")
...
def develop_model(prepared_data: pd.DataFrame, target_column: str, positive_label: str, negative_label: str) -> (any, dict, dict):
    ...
    train_metrics = measure_model_performance(model, train_feature, train_target, 'Training', positive_label, negative_label)

    logging.info("Measuring model performance for testing data")
    test_metrics = measure_model_performance(model, test_feature, test_target, 'Testing', positive_label, negative_label)
